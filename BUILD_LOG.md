...
## Session 77 — Energy Budget Chart
**Date:** 2026-03-17

### What I Built
**Energy Budget Chart** — a stacked area chart showing ecosystem energy flow over time, revealing the thermodynamics of the simulation. Three energy pools are tracked simultaneously, with an efficiency overlay.

### Energy Layers
- **Biomass** (cyan area): Total organism energy = population × average energy per organism. Shows the living energy reservoir.
- **Food Energy** (green area): Available food × 30 energy units. Shows unconsumed resources waiting to be harvested.
- **Death Toll** (red line): Cumulative deaths × average energy, representing energy leaving the system through mortality.
- **Efficiency** (yellow dashed): Biomass / (Biomass + Food) — the fraction of total ecosystem energy captured by life. High efficiency means organisms are consuming most available resources.

### Visual Features
- Stacked filled areas show biomass on top of food — total height = total system energy
- Current frame vertical marker for temporal reference
- Current values header: "Biomass: N  Food: N  Eff: N%"
- Dual Y-axis: left = absolute energy, right = efficiency percentage (0-100%)
- Frame-synced with all other charts

### What It Reveals
- **Paradise**: Food energy dominates early, efficiency rises as population grows to consume it all
- **Apocalypse**: Wild biomass fluctuations, efficiency crashes during extinctions
- **Ecosystem**: Oscillating efficiency as predator-prey cycles convert food → biomass → death
- **Gladiator**: Rapid biomass collapse as combat drains the energy pool

### Stats
- Tests: 193 passed, 0 failed
- Replays: 15 regenerated
- Session counter: 77
- Total analytical charts: 16

### Next session ideas
- Replay comparison mode (load two replays side-by-side)
- Sound evolution — pitch/rhythm drift with genome changes
- Organism family tree export (SVG/PNG of full lineage)
- Combat network graph (who killed whom, weighted edges)

---

## Session 78 — Combat Network Graph
**Date:** 2026-03-17

### What I Built

**1. Individual Kill Tracking (Backend)**
Added `killed_by` field to the Organism class, tracking which organism killed each victim. Both `_fight()` and `_pack_fight()` now record the killer's ID. The graveyard stores this as `"killer"`, and the exporter creates a `combat_log` array of `{k: killerID, v: victimID, t: tick}` entries.

**2. Combat Network Graph (Viewer)**
A force-directed graph visualization showing individual kill relationships. Each node is an organism involved in combat (limited to top 40 most involved), each directed edge represents a kill relationship.

### Visual Features
- **Force-directed layout**: 120-iteration spring simulation positions nodes automatically — connected nodes cluster together, same-species organisms attract gently
- **Directed edges**: Red arrows from killer to victim, thickness proportional to kill count
- **Node sizing**: Radius scales with kill count (prolific killers are larger)
- **Node coloring**: Uses organism's actual RGB color from genome
- **Glow effect**: Nodes with 3+ kills have a subtle halo
- **Kill count labels**: Displayed below prolific killers (2+ kills)
- **Interactive**: Click any node to track that organism in the main viewer
- **Tracked highlight**: White border on the currently tracked organism's node

### Force Layout Algorithm
- **Repulsion**: All nodes repel each other (F = 800/d²)
- **Edge attraction**: Connected nodes attract toward ideal distance of 60px (F = (d-60)×0.01)
- **Species clustering**: Same-species nodes attract gently (F = (d-40)×0.003)
- **Damping**: 0.85 velocity damping per iteration, 120 iterations total
- **Capping**: Top 40 most-involved organisms to keep graph readable

### New Tests (2)
- `test_killed_by_tracks_killer`: Verifies combat sets killed_by to killer's ID
- `test_graveyard_has_killer`: Verifies graveyard entries include killer field

### Bug Fix
Found and fixed: `_pack_fight()` was missing `killed_by` assignment — only `_fight()` had it initially. Both pack combat and individual combat now properly track killers.

### Stats
- Tests: 195 passed, 0 failed (2 new)
- Replays: 15 regenerated
- Session counter: 78
- Total analytical charts: 17

### Next session ideas
- Replay comparison mode (load two replays side-by-side)
- Sound evolution — pitch/rhythm drift with genome changes
- Organism family tree export (SVG/PNG of full lineage)
- Territorial control heatmap over time (animated species territory evolution)
